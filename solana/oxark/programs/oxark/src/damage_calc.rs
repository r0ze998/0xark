...
fn apply_action_types(p1: &mut [CardWork; 5], p2: &mut [CardWork; 5], seed: &[u8; 32]) {
    ...
    for i in 0..5 {
        if void_p1[i] {
            p1[i].void_blocked = true;
        }
    }
    for i in 0..5 {
        if void_p2[i] {
            p2[i].void_blocked = true;
        }
    }
    ...
    for &(side, idx) in &order {
        ...
        match at {
            ...
            1 => {
                // Barrier: set barrierUp
                if side == 0 {
                    p1[idx].barrier_up = true;
                } else {
                    p2[idx].barrier_up = true;
                }
            }
            2 => {
                // UseFlame: -5 HP to opposing card at same slot
                if side == 0 {
                    if idx < 5 {
                        p2[idx].hp_mod -= 5;
                    }
            ...
            3 => {
                // UseStorm: all opposing cards -2 BP, clear their barriers
                if side == 0 {
                    for c in p2.iter_mut() {
                        c.bp_mod -= 2;
                        c.barrier_up = false;
            ...
            4 => {
                // UseShadow: self.shadow = true (skip pair)
                if side == 0 {
                    p1[idx].shadow = true;
                } else {
                    p2[idx].shadow = true;
...
fn run_combat(p1: &mut [CardWork; 5], p2: &mut [CardWork; 5], seed: &[u8; 32]) {
    for i in 0..5 {
        ...
        let a_first =
            p1[i].ini > p2[i].ini || (p1[i].ini == p2[i].ini && seed_byte(seed, 24 + i) < 128);

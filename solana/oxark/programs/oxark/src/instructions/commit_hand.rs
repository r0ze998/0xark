...
use crate::groth16::{self, Groth16Vk};
...
const VK_HC_ALPHA_G1: [u8; 64] = [
    13, 58, 172, 32, 177, 216, 235, 208, 183, 197, 110, 70, 31, 229, 53, 155, 63, 214, 137, 1, 218,
    228, 83, 211, 21, 131, 44, 40, 52, 13, 123, 187, 40, 111, 8, 171, 21, 188, 128, 206, 167, 191,
    183, 30, 41, 233, 189, 48, 58, 111, 49, 217, 63, 251, 80, 241, 160, 228, 15, 209, 219, 150,
    238, 66,
];

const VK_HC_BETA_G2: [u8; 128] = [
    42, 235, 231, 89, 194, 244, 191, 107, 20, 166, 221, 165, 96, 69, 244, 75, 116, 216, 250, 209,
    156, 90, 171, 236, 166, 226, 144, 63, 137, 230, 112, 51, 18, 78, 231, 105, 214, 69, 237, 7,
    200, 253, 80, 199, 198, 126, 188, 220, 116, 2, 244, 57, 125, 203, 217, 78, 162, 98, 242, 82,
    35, 68, 185, 252, 25, 18, 34, 172, 185, 14, 120, 142, 67, 49, 175, 48, 232, 205, 34, 213, 234,
    164, 102, 93, 103, 143, 1, 69, 151, 166, 5, 55, 71, 147, 137, 54, 28, 211, 42, 28, 34, 168, 29,
    207, 111, 80, 158, 142, 60, 206, 183, 67, 218, 45, 200, 53, 121, 246, 252, 97, 148, 244, 61,
    38, 115, 142, 176, 84,
];
...
const VK_HC_DELTA_G2: [u8; 128] = [
    28, 19, 210, 173, 36, 148, 143, 250, 29, 242, 173, 209, 159, 185, 144, 103, 154, 210, 71, 88,
    34, 178, 204, 40, 214, 24, 122, 134, 88, 160, 208, 222, 9, 125, 133, 35, 110, 109, 101, 184,
    56, 3, 250, 98, 209, 209, 56, 102, 113, 33, 163, 203, 163, 80, 242, 14, 133, 232, 61, 119, 120,
    229, 211, 90, 47, 215, 79, 0, 242, 195, 157, 218, 147, 183, 247, 52, 80, 64, 249, 21, 228, 96,
    196, 192, 142, 84, 163, 10, 55, 195, 72, 205, 246, 117, 108, 225, 22, 117, 241, 234, 6, 194,
    26, 84, 171, 156, 193, 202, 139, 140, 29, 157, 90, 64, 177, 146, 242, 77, 85, 154, 23, 40, 14,
    231, 141, 152, 211, 34,
];

/// IC[0] — constant contribution
const VK_HC_IC0: [u8; 64] = [
    27, 174, 203, 67, 248, 178, 60, 199, 92, 58, 207, 28, 196, 127, 75, 92, 166, 44, 180, 134, 247,
    198, 244, 71, 107, 11, 254, 162, 72, 53, 209, 225, 45, 68, 54, 243, 200, 213, 168, 149, 53, 38,
    121, 182, 10, 125, 112, 10, 19, 130, 123, 132, 234, 25, 54, 156, 246, 151, 43, 155, 204, 71,
    228, 205,
];

/// IC[1] — commitment (output signal, index 0 in public signals)
const VK_HC_IC1: [u8; 64] = [
    32, 248, 112, 79, 73, 130, 127, 157, 108, 236, 118, 47, 250, 192, 68, 151, 48, 144, 78, 254,
    17, 73, 184, 43, 32, 195, 201, 9, 95, 234, 222, 77, 46, 231, 59, 226, 228, 68, 239, 129, 149,
    181, 151, 159, 83, 68, 34, 134, 20, 178, 36, 132, 73, 114, 45, 198, 194, 164, 94, 212, 183, 87,
    249, 155,
];

/// IC[2] — round (public input 0)
const VK_HC_IC2: [u8; 64] = [
    42, 189, 220, 242, 234, 227, 222, 10, 243, 179, 73, 160, 6, 186, 242, 142, 170, 220, 35, 62,
    43, 142, 65, 228, 11, 34, 135, 173, 211, 182, 118, 187, 12, 87, 99, 91, 181, 98, 123, 9, 49, 8,
    209, 26, 211, 71, 248, 1, 127, 179, 85, 80, 97, 211, 118, 209, 213, 51, 19, 40, 227, 126, 229,
    91,
];

/// IC[3] — pubkey_lo (public input 1)
const VK_HC_IC3: [u8; 64] = [
    45, 3, 235, 249, 24, 39, 107, 12, 142, 187, 30, 175, 188, 205, 7, 113, 214, 254, 240, 125, 235,
    128, 147, 13, 110, 101, 59, 74, 37, 146, 189, 248, 31, 207, 128, 61, 232, 105, 236, 60, 222,
    86, 5, 49, 152, 48, 244, 198, 93, 47, 129, 173, 116, 228, 100, 59, 44, 75, 237, 174, 244, 162,
    209, 44,
];

/// IC[4] — pubkey_hi (public input 2)
const VK_HC_IC4: [u8; 64] = [
    13, 88, 114, 166, 177, 44, 206, 223, 156, 166, 114, 167, 16, 204, 102, 212, 150, 168, 225, 44,
    138, 67, 136, 154, 97, 9, 149, 215, 67, 213, 36, 210, 23, 22, 124, 7, 24, 46, 77, 133, 108, 6,
    5, 40, 168, 156, 177, 30, 85, 241, 9, 37, 167, 21, 138, 150, 47, 68, 79, 168, 152, 40, 68, 41,
];

/// VK points used by the pairing check (math lives in crate::groth16).
const VK_HC: Groth16Vk<'static> = Groth16Vk {
    alpha_g1: &VK_HC_ALPHA_G1,
    beta_g2: &VK_HC_BETA_G2,
    gamma_g2: &VK_HC_GAMMA_G2,
    delta_g2: &VK_HC_DELTA_G2,
};
...
pub fn handle_commit_hand(
    ...
    let vk_x = groth16::compute_vk_x(
        &VK_HC_IC0,
        &[&VK_HC_IC1, &VK_HC_IC2, &VK_HC_IC3, &VK_HC_IC4],
        &public_signals,
    )?;
    let valid = groth16::groth16_verify(&VK_HC, &proof_a, &proof_b, &proof_c, &vk_x)?;

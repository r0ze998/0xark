...
const VK_DELTA_G2: [u8; 128] = [
    20, 240, 89, 229, 101, 154, 207, 46, 168, 230, 92, 111, 116, 62, 55, 252, 225, 97, 224, 195,
    173, 34, 197, 220, 199, 120, 17, 126, 27, 86, 20, 13, 21, 20, 8, 190, 164, 119, 233, 63, 113,
    32, 93, 33, 251, 34, 209, 162, 26, 8, 193, 57, 123, 206, 173, 44, 75, 128, 160, 222, 249, 48,
    31, 73, 2, 1, 65, 126, 69, 226, 241, 86, 144, 225, 216, 134, 132, 231, 9, 148, 133, 88, 29,
    153, 10, 248, 158, 66, 247, 17, 181, 133, 186, 101, 177, 213, 12, 236, 47, 50, 26, 33, 233,
    125, 35, 157, 116, 80, 209, 54, 41, 203, 127, 57, 118, 41, 233, 58, 191, 254, 126, 49, 161, 35,
    111, 136, 41, 137,
];

/// IC[0] — contribution from constant 1
const VK_IC0: [u8; 64] = [
    29, 196, 105, 161, 42, 10, 42, 5, 24, 115, 12, 126, 127, 151, 177, 41, 116, 76, 103, 240, 155,
    94, 102, 167, 7, 216, 146, 78, 7, 219, 72, 230, 25, 212, 215, 36, 168, 132, 71, 52, 7, 114,
    188, 122, 217, 54, 51, 248, 12, 6, 190, 149, 28, 63, 159, 103, 123, 180, 46, 100, 96, 3, 118,
    174,
];

/// IC[1] — contribution from public_inputs[0] = commitment
const VK_IC1: [u8; 64] = [
    37, 60, 27, 104, 10, 234, 113, 152, 104, 245, 168, 107, 237, 203, 1, 57, 128, 254, 81, 25, 134,
    237, 156, 119, 233, 228, 136, 91, 174, 239, 69, 216, 39, 105, 225, 178, 36, 73, 212, 27, 45,
    209, 76, 51, 146, 203, 150, 37, 253, 110, 186, 63, 239, 35, 83, 242, 17, 197, 25, 160, 199,
    231, 39, 208,
];

/// IC[2] — contribution from public_inputs[1] = round
const VK_IC2: [u8; 64] = [
    46, 113, 129, 70, 61, 211, 128, 155, 82, 142, 208, 81, 123, 239, 154, 2, 228, 165, 63, 45, 167,
    216, 232, 196, 114, 127, 56, 101, 229, 185, 187, 21, 0, 97, 59, 83, 64, 16, 144, 9, 185, 25,
    50, 242, 143, 221, 198, 120, 123, 39, 97, 32, 25, 92, 243, 138, 107, 68, 109, 190, 253, 235,
    205, 234,
];

/// IC[3] — contribution from public_inputs[2] = pubkey_lo
const VK_IC3: [u8; 64] = [
    29, 107, 142, 110, 48, 246, 148, 225, 177, 104, 205, 85, 213, 141, 213, 148, 247, 208, 101, 95,
    158, 26, 208, 149, 35, 30, 220, 173, 174, 80, 228, 173, 44, 62, 253, 165, 254, 154, 6, 22, 47,
    208, 51, 52, 247, 43, 95, 35, 222, 113, 194, 43, 96, 235, 79, 76, 226, 82, 164, 77, 100, 109,
    197, 125,
];

/// IC[4] — contribution from public_inputs[3] = pubkey_hi
const VK_IC4: [u8; 64] = [
    26, 10, 170, 21, 171, 232, 1, 4, 207, 226, 157, 145, 149, 73, 31, 38, 70, 204, 68, 244, 242,
    75, 32, 226, 50, 162, 28, 57, 141, 88, 119, 38, 13, 233, 4, 126, 8, 95, 117, 165, 58, 31, 191,
    181, 102, 233, 117, 17, 40, 38, 40, 103, 123, 206, 105, 246, 221, 75, 243, 156, 33, 149, 189,
    178,
];

/// VK points used by the pairing check (math lives in crate::groth16).
const VK: Groth16Vk<'static> = Groth16Vk {
...
pub fn handle_verify_zk(
    ...
    let vk_x = groth16::compute_vk_x(
        &VK_IC0,
        &[&VK_IC1, &VK_IC2, &VK_IC3, &VK_IC4],
        &public_inputs,
    )?;
    let valid = groth16::groth16_verify(&VK, &proof_a, &proof_b, &proof_c, &vk_x)?;

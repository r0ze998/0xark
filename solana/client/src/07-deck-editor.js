...
// Frontend only (T-D5-2). No save/load — real NFT integration in T-D5-3.
// ═══════════════════════════════════════════════════════════════════════════

// ── State ────────────────────────────────────────────────────────────────
let deckEditorOpen   = false;
let deckEditorFilter = 'all'; // 'all' | 'attack' | 'defense' | 'flee' | 'magic' | 'recovery' | clan
...
// Placeholder storage (30 cards from CD for demo — T-D5-3 replaces with real NFTs)
let deckStorageCards = [];
function _initDeckStorage() {
  if (deckStorageCards.length) return;
  // Use first 30 cards, give them varied quantities
  for (let i = 1; i <= 30; i++) {
    deckStorageCards.push({ card_id: i, qty: Math.ceil(Math.random() * 2 + 1) });
  }
}
...
// Wire close to PC Box dialog
// lobbyDialogConfirm() calls this when action === 'open_deck_editor'
// (button is disabled for now; future: enable when T-D5-3 is live)
...
function drawDeckEditor() {
  ...
  g.fillStyle = '#1a2a1a';
  g.fillRect(ow - DE_PAD - 130, barY + 3, 124, DE_BTN_H - 6);
  g.strokeStyle = '#2a4a2a';
  g.lineWidth = 1;
  g.strokeRect(ow - DE_PAD - 130, barY + 3, 124, DE_BTN_H - 6);
  g.fillStyle = '#3a5a3a';
  g.font = '14px VT323, monospace';
  g.textAlign = 'center';
  g.fillText('Save Deck (T-D5-3)', ow - DE_PAD - 68, barY + 19);

  // Hint
  g.fillStyle = '#444466';
  g.font = '10px VT323, monospace';
  g.textAlign = 'left';
  g.fillText('Click card in storage to add · Click deck slot to remove · Arrow keys scroll storage', DE_PAD + 4, barY + 19);
}

...
export function computePostBattleImprints({
  ...
  const isDestroyed = (card, destSet) =>
    card.destroyed || destSet.has(card.id);

  // FirstBlood: first win ever
  if (isFirstWin) imprints.push({ key: 'FirstBlood', description: 'First battle victory', is_cosmetic: false, value: 1 });

  // FlawlessVictory: none of own cards destroyed
  ...
  // LegendaryDefeat: defeated at least one Legendary opponent card
  const killedLgd = oppCards.some(c => (c.rarity === 3 || c.isLegendary) && isDestroyed(c, oppDestIds));
  if (killedLgd) {

...
function _drawLockInButton(rx) {
  ...
  // Scout peek button (above lock in, only in summon phase)
  if (DS.phase === 'summon' && DS.scoutPeekUsed < SCOUT_PEEK_MAX) {
    const sy = by - 18;
    _drawSmallButton(rx + 4, sy, DL.RP_W - 8, 16, `PEEK (${3-DS.scoutPeekUsed} left)`, '#5080a0', true);
  }
}
...
function _handlePanelInput(px, py) {
  ...
  // Scout Peek button (above Lock In)
  if (DS.phase === 'summon' && DS.scoutPeekUsed < SCOUT_PEEK_MAX) {
    const sy = DL.LOCKIN_Y - 18;
    if (py >= sy && py < sy + 16) {
      _triggerScoutPeek();
      return;
    }

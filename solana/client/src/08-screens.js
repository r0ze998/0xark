...
function drawFloorClearFanfare(){
  ...
  if(d.isGoal){
    ...
    // Extra star burst — v393: sin-addition + hoist dist (was sin called 8+8=16 times/frame)
    {const _s01=Math.sin(t*0.1),_c01=Math.cos(t*0.1);
    const _sd=52+Math.sin(t*0.08)*6; // hoisted: was inside loop (same value every iteration)
    const _s12=Math.sin(t*0.12),_c12=Math.cos(t*0.12);
    for(let si=0;si<8;si++){
      const _ac=_c01*_ORB_CI8[si]-_s01*_ORB_SI8[si],_as=_s01*_ORB_CI8[si]+_c01*_ORB_SI8[si];
      g.globalAlpha=alpha*(0.4+(_s12*_IDX_CI[si]+_c12*_IDX_SI[si])*0.3);
      bx(px_+pw/2+_ac*_sd-2,py_+34+_as*10-2,4,4,'#ffe080');
    }}
    g.globalAlpha=alpha;
    ...
    g.globalAlpha=alpha;
    // Epic/Legendary sparkles — v393: sin-addition, hoist dist, use ORB tables
    {const _nc=rar===5?6:4;
    const _s15=Math.sin(t*0.15),_c15=Math.cos(t*0.15);
    const _sd=36+Math.sin(t*0.09)*4; // hoisted from loop
    const _s13=Math.sin(t*0.13),_c13=Math.cos(t*0.13);
    const _orbSI=rar===5?_ORB_SI6:_ORB_SI10,_orbCI=rar===5?_ORB_CI6:_ORB_CI10;
    // N=4 uses ORB_SI10[0,2,5,7] (approx), but simplest: use Euler recurrence
    const _step=Math.PI*2/_nc,_ss=Math.sin(_step),_sc=Math.cos(_step);
    let _si=0,_ci=1; // starting at i=0: sin(0)=0, cos(0)=1
    for(let si=0;si<_nc;si++){
      const _as=_s15*_ci+_c15*_si,_ac=_c15*_ci-_s15*_si; // sin/cos(t*0.15+si*step)
      g.globalAlpha=alpha*(0.5+(_s13*_IDX_CI[si]+_c13*_IDX_SI[si])*0.25);
      bx(px_+pw/2+_ac*_sd-2,py_+26+_as*8-2,3,3,rcol);
      const _ns=_si*_sc+_ci*_ss;_ci=_ci*_sc-_si*_ss;_si=_ns; // Euler advance
    }}
    g.globalAlpha=alpha;
  ...
  // Rarity orbit sparks — v393: sin-addition + Euler recurrence + hoist dist
  const sparkCount=d.isGoal?12:2+rar*2;
  const sparkDist=d.isGoal?52:38;
  {const _tK=t*(d.isGoal?0.08:0.12);
  const _sK=Math.sin(_tK),_cK=Math.cos(_tK);
  const _dist=sparkDist+Math.sin(t*0.1)*4; // hoisted from loop
  const _sq=d.isGoal?0.9:0.75,_sz=d.isGoal?5:4;
  const _step=Math.PI*2/sparkCount,_ss=Math.sin(_step),_sc=Math.cos(_step);
  let _si=0,_ci=1;
  for(let i=0;i<sparkCount;i++){
    const _ac=_cK*_ci-_sK*_si,_as=_sK*_ci+_cK*_si;
    g.globalAlpha=alpha*_sq;
    bx(cX+cw_/2+_ac*_dist-2,cY+ch_/2+_as*_dist-2,_sz,_sz,rcol);
    const _ns=_si*_sc+_ci*_ss;_ci=_ci*_sc-_si*_ss;_si=_ns;
  }}
  g.globalAlpha=alpha;

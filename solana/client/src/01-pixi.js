...
// T52: Low-frequency tension rumble — plays when rival is 2 tiles away (pre-encounter)
function sfxTensionRumble(){if(!soundEnabled)return;try{const s=AC.createBufferSource(),gn=AC.createGain(),f=AC.createBiquadFilter();s.buffer=noiseBuf;f.type='lowpass';f.frequency.setValueAtTime(180,AC.currentTime);f.Q.value=2;gn.gain.setValueAtTime(0,AC.currentTime);gn.gain.linearRampToValueAtTime(.04,AC.currentTime+.15);gn.gain.linearRampToValueAtTime(0,AC.currentTime+.55);s.connect(f);f.connect(gn);gn.connect(AC.destination);s.start();s.stop(AC.currentTime+.55);}catch(e){}}

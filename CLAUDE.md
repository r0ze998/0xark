...
| `solana/client/oxark-idl.json` | 63 | ⚠️ Stale (manual sync only — NOT regenerated by `anchor build`) |

`solana/client/oxark-idl.json` is 3 behind (missing `settle_duel_history`,
`claim_timeout_win`, `refill_energy`). This does NOT block the client: `onchain.js`
hand-builds instruction data via `disc()` and never reads the IDL at runtime, and it
already wraps all four newer instructions. Resync for tooling hygiene with
`cp solana/oxark/target/idl/oxark.json solana/client/oxark-idl.json` (see the devnet
runbook §6b). All three contain Program ID `5i37jWBiA7bV9XmokyDWHQxjJ5s1sBnSEkPSB4J2XfmN`.

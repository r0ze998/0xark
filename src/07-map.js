  ...
  if(currentMap===0){
    ...
    if(fogRevealed[1][chestTY]?.[chestTX]){
      ...
      if(cpx>-TW&&cpx<W+TW&&cpy>-TH&&cpy<H+TH){
        ...
        // v533: treasure chest golden particle fountain (world coords, next-frame display)
        if(fr%6===0){const _cgx=chestTX*TW+12+Math.random()*8,_cgy=chestTY*TH+8+Math.random()*4;particles.push({x:_cgx,y:_cgy,vx:(Math.random()-.5)*0.8,vy:-0.6-Math.random()*1.0,life:14+Math.random()*10,c:Math.random()>.6?'rgba(255,220,50,0.85)':'rgba(255,255,180,0.65)'});}
      }
    }
      ...
      for(let gx=Math.max(0,Math.floor(camX/TW));gx<=Math.min(MW-1,Math.ceil((camX+W)/TW));gx++){
        ...
        if((gt===25||gt===26)&&fogRevealed[2][gy]?.[gx]){
          ...
          if(gpx>-20&&gpx<W+20&&gpy>-20&&gpy<H+20){
            ...
            g.globalAlpha=1;
            // v533: lava/crystal tile rising sparks — tile-offset frame distribution (world coords, next-frame)
            if(fr%30===(gx*3+gy*7)%30){if(gt===25){particles.push({x:gx*TW+8+Math.random()*16,y:gy*TH+8+Math.random()*8,vx:(Math.random()-.5)*0.7,vy:-0.5-Math.random()*0.8,life:12+Math.random()*9,c:Math.random()>.5?'rgba(240,100,30,0.72)':'rgba(255,200,60,0.55)'});}else{particles.push({x:gx*TW+8+Math.random()*16,y:gy*TH+6+Math.random()*8,vx:(Math.random()-.5)*0.5,vy:-0.4-Math.random()*0.7,life:14+Math.random()*10,c:Math.random()>.5?'rgba(60,160,240,0.68)':'rgba(180,230,255,0.50)'});}}
          }
        }
      }
    }

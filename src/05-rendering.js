...
function drawMountain(px,py,tx_,ty){
  ...
  // Craftpix ruins: ruins2(idx1)=80×80 → scale 0.4 → 32×32, ruins1(idx0)=112×128 → scale 0.25 → 28×32
  { const h=tileHash(tx_,ty);
    const sv=((h&7)-4);
    bx(px,py,TW,TH,`rgb(${110+sv},${108+sv},${124+sv})`);
    const rIdx=(h&3)===0?0:1; // ruins1 or ruins2 for variety
    const sc=rIdx===0?0.25:0.4;
    const ox=rIdx===0?2:0, oy=rIdx===0?0:0;
    if(drawCpxRuin(rIdx,px+ox,py+oy,sc)){
      // shadow under
      g.fillStyle='rgba(0,0,0,.2)';g.beginPath();g.ellipse(px+16,py+TH-2,14,4,0,0,Math.PI*2);g.fill();
...
function drawRock(px,py,tx_,ty){
  drawGrass(px,py,tx_,ty);
  // Craftpix Blue-gray ruins: ruins5=32x32, ruins4=48x48
  const rIdx=(tileHash(tx_,ty)%2===0)?4:3; // ruins5 or ruins4
...
function drawRuinsWall(px,py,tx_,ty){
  // Craftpix: ruins3(idx2)=48×64 → scale 0.5 → 24×32, ruins2(idx1)=80×80 → scale 0.4 → 32×32
  { const h=tileHash(tx_,ty);
    const rIdx=(h&1)===0?2:1; // ruins3 or ruins2
    const sc=rIdx===2?0.5:0.4;
    const ox=rIdx===2?4:0; // center ruins3 (24px wide → offset 4)
    bx(px,py,TW,TH,'#5c5c64');
    if(drawCpxRuin(rIdx,px+ox,py,sc)){
      const r=tr(tx_,ty);
      if(r.e>.5){bx(px+Math.floor(r.f*20)+2,py+TH-6,8,5,'#486848');}
      bx(px,py,TW,TH,'rgba(0,0,20,.12)');
      return;
    }
  }
  if(useKenney.ruins){

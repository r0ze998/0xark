  ...
  for(let y=startTY;y<=endTY;y++){
    for(let x=startTX;x<=endTX;x++){
      ...
      if(t===31){
        ...
        g.globalAlpha=1;
        // v519: rising blue mist from stair-up opening (world coords)
        if((fr+(x*5+y*11))%5===0){
  ...
  for(let y=startTY;y<=endTY;y++){
    for(let x=startTX;x<=endTX;x++){
      ...
      if(t===29){
        ...
        // v514: rising embers from campfire (world coords)
        if((fr+(x*9+y*5))%6===0){
          const _ewx=x*TW+10+Math.random()*12,_ewy=y*TH+2+Math.random()*6;
          const _ec=Math.random()>.6?'rgba(255,160,40,0.9)':Math.random()>.4?'rgba(255,80,20,0.8)':'rgba(255,220,80,0.7)';
          particles.push({x:_ewx,y:_ewy,vx:(Math.random()-.5)*0.8,vy:-0.8-Math.random()*1.0,life:18+Math.random()*14,c:_ec});
        }
        continue;
      }
      if(t===25){
      ...
      if(t===24){
        ...
        // v518: drifting teal motes from glow tile (world coords)
        if((fr+(x*19+y*9))%9===0){
          const _gwx=x*TW+4+Math.random()*24,_gwy=y*TH+4+Math.random()*24;
          particles.push({x:_gwx,y:_gwy,vx:(Math.random()-.5)*0.5,vy:-0.2-Math.random()*0.4,life:28+Math.random()*20,c:Math.random()>.5?'rgba(40,200,160,0.55)':'rgba(100,240,200,0.4)'});
        }
      }else if(t===26){
        ...
        // v517: crystal sparkle particles — occasional burst of cyan/white motes (world coords)
        if((fr+(x*17+y*13))%12===0){
          const _csx=x*TW+8+Math.random()*16,_csy=y*TH+4+Math.random()*16;
          particles.push({x:_csx,y:_csy,vx:(Math.random()-.5)*0.7,vy:-0.4-Math.random()*0.7,life:14+Math.random()*14,c:Math.random()>.5?'rgba(120,220,255,0.8)':'rgba(220,240,255,0.65)'});
        }
      }else if(t===27){
...
function drawTownAnimatedOverlays(startTX,startTY,endTX,endTY){
  ...
  // v520: town ambient drift — flower petals and dandelion seeds float on the breeze (world coords)
  if(fr%8===0){
    const _tawx=camX+Math.random()*W,_tawy=camY+Math.random()*(H-80);
    const _tar=Math.random();
    const _tac=_tar>.6?'rgba(255,180,200,0.5)':_tar>.3?'rgba(255,240,180,0.45)':'rgba(220,230,255,0.4)';
    particles.push({x:_tawx,y:_tawy,vx:0.4+Math.random()*0.6,vy:-0.05-Math.random()*0.2,life:55+Math.random()*35,c:_tac});
  }
  // v534: shooting star — rare celestial streak across the sky (world coords, fires ~every 12s)
  if(Math.floor(fr/2)%360===0){
    const _ssx=camX+20+Math.random()*(W-60),_ssy=camY+8+Math.random()*36;
    const _svx=3.5+Math.random()*2.5,_svy=1.0+Math.random()*0.8;
    for(let _si=0;_si<7;_si++){particles.push({x:_ssx-_si*_svx*1.6,y:_ssy-_si*_svy*1.6,vx:_svx,vy:_svy,life:8+_si,c:`rgba(255,255,240,${(0.9-_si*0.11).toFixed(2)})`});}
  }
  // v534: palm tree shimmer — golden motes drift from palm canopies (tile 14, world coords)
  if(fr%4===0){const _pm=getMap();
    for(let _py=startTY;_py<=endTY;_py++){for(let _px=startTX;_px<=endTX;_px++){
      if(_pm[_py]?.[_px]!==14)continue;
      if(!fogRevealed[currentMap]?.[_py]?.[_px])continue;
      if((fr/4+_px*3+_py*7)%5!==0)continue;
      particles.push({x:_px*TW+6+Math.random()*20,y:_py*TH-4+Math.random()*8,vx:(Math.random()-.5)*0.8,vy:-0.2-Math.random()*0.4,life:22+Math.random()*16,c:Math.random()>.5?'rgba(200,230,100,0.45)':'rgba(255,240,150,0.38)'});
    }}
  }
}

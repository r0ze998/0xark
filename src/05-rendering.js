...
function drawGlowTile(px,py,tx_,ty){
  ...
  g.globalAlpha=pulse*.6;g.drawImage(_glowTileArc[(tx_+ty)%2],(px+11)|0,(py+11)|0);g.globalAlpha=1;
}
...
function drawDungeonStairGlows(startTX,startTY,endTX,endTY){
  ...
  for(let y=startTY;y<=endTY;y++){
    for(let x=startTX;x<=endTX;x++){
      ...
      if(t===31){
        // Down staircase glow — pre-baked ellipses, only arrowCol needs template literal
        const pulse=Math.sin(fr*.07+x*2+y*3)*.25+.75;
        const isGold=currentMap>=5;
        const arrowCol=isGold?`rgba(255,240,80,${pulse*.9})`:`rgba(200,160,255,${pulse*.8})`;
        ...
        // Up staircase glow — pre-baked ellipses, only arrowCol needs template literal

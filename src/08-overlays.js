...
function drawMarketplace(){
  ...
  if(marketTab===0){
    // MY CARDS — vault collection grouped by type
    ...
    typeFilter.forEach((typ,ti)=>{
      ...
      txShadow(typeLabel[ti]+' ('+cards.length+')',mx+14,yi+10,7,typeColor[ti],'rgba(0,0,0,.3)');yi+=16;
    ...
    mockListings.forEach((listing,li)=>{
      const cr=CD[listing.card-1];if(!cr)return;

...
// v397: pre-baked spatial phases for map load screen dungeon particles (30 entries, step 0.7)
...
// v281: hoisted MP player sprite colors
const _MP_SPRITE_COLS=['#d860a0','#d8b028','#48b8e8'];
// Draw other multiplayer players on map as real sprites
function drawMPPlayers(){
  if(!mp.connected||mp.phase!=='playing')return;
  for(let _mpi=0;_mpi<mp.otherPlayers.length;_mpi++){
    const op=mp.otherPlayers[_mpi];
...
function drawMapLoadScreen(){
  ...
  if(isDungeon){
    ...
    // v397: sin-addition with pre-baked spatial phases (30→2 trig calls/frame)
    {const _mS8=Math.sin(mapLoadScreenFrame*0.08),_mC8=Math.cos(mapLoadScreenFrame*0.08);
    for(let i=0;i<30;i++){
      const px_=((i*1237+mapLoadScreenFrame*3)%1000/1000)*W;
      const py_=((i*3141+mapLoadScreenFrame*4)%1000/1000)*(H+40)-20;
      const pa=0.3+(_mS8*_MLDS_CI07[i]+_mC8*_MLDS_SI07[i])*0.3;
    ...
    if(t>0.38&&t<0.90){
      ...
      if(rivalMaps[0]===mapLoadScreenFloor){
        ...
        g.globalAlpha=alpha*warnAlpha*0.9;g.fillStyle='#e060a0';g.shadowColor='#e060a0';g.shadowBlur=8;
        g.fillText(_wTxt,W/2,_rhBaseY+_rhN*14);g.shadowBlur=0;_rhN++;
      }
      if(rivalMaps[1]===mapLoadScreenFloor){
        const _wTxt='\u26a0 '+pl[2].n+': '+cdCount(pl[2].cd)+' cards \u2014 HERE';
        g.globalAlpha=alpha*warnAlpha*0.9;g.fillStyle='#d0a030';g.shadowColor='#d0a030';g.shadowBlur=8;
        g.fillText(_wTxt,W/2,_rhBaseY+_rhN*14);g.shadowBlur=0;_rhN++;
      }
      g.textAlign='left';g.globalAlpha=alpha;
    }
...
function updateRunSummary(){
  ...
  runSummaryFrame++;
  // v488: one-shot sparkle on reveal when all cards preserved
  if(runSummaryFrame===32&&runSummaryData&&runSummaryData.lostCards.length===0){
    const _rx=W/2,_ry=H/2+20;
    for(let _rsi=0;_rsi<20;_rsi++){const _rsa=(_rsi/20)*Math.PI*2+Math.random()*0.4;const _rss=1.5+Math.random()*3;
...
function drawRunSummary(){
  ...
  if(d.lostCards.length>0){
    ...
    for(let _lci=0;_lci<d.lostCards.length;_lci++){
      const cr=d.lostCards[_lci],ly=py+107+_lci*18;

...
function tryMovePlayer(dx, dy) {
  ...
  // v525: flower tile petal burst — pink/yellow petals mix with grass particles
  if(newTile===7&&!inDungeon){
    const _fwx=nx*TW,_fwy=ny*TH+TH-4;
    for(let _fi=0;_fi<2;_fi++){
      particles.push({x:_fwx+6+Math.random()*20,y:_fwy,vx:(Math.random()-.5)*1.4,vy:-0.6-Math.random()*1.0,life:24+Math.random()*12,c:Math.random()>.5?'rgba(255,180,200,0.85)':'rgba(255,240,120,0.8)'});
    }
  }
  // v217→v525: dungeon footstep dust — all 5 floors have distinct atmospheric colors
  if(inDungeon&&(newTile===1||newTile===4||newTile===10)){
    const dustX=nx*TW,dustY=ny*TH+TH-4;
    const dustN=currentFloor>=5?5:currentFloor===4?4:3;
    const _dc1=currentFloor>=5?'rgba(255,200,60,1)':currentFloor===4?'rgba(200,70,30,1)':currentFloor===3?'rgba(180,50,160,1)':currentFloor===2?'rgba(100,50,180,1)':'rgba(70,90,150,1)';
    const _dc2=currentFloor>=5?'rgba(80,240,220,1)':currentFloor===4?'rgba(255,140,40,1)':currentFloor===3?'rgba(255,100,220,1)':currentFloor===2?'rgba(160,100,255,1)':'rgba(120,140,200,1)';
    for(let i=0;i<dustN;i++){
      particles.push({x:dustX+4+Math.random()*TW/2,y:dustY,vx:(Math.random()-.5)*1.2,vy:-0.8-Math.random()*0.6,life:14+Math.random()*8,c:Math.random()>.5?_dc1:_dc2});
    }
  }

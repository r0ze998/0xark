...
function drawBattleBG(){
  ...
  if(currentMap===0&&!inDungeon){
    ...
    g.globalAlpha=1;}
    // v526: floor-specific battle ambient particles (screen-space; drawn via drawParticles(0,0))
    {const _aRate=fl>=4?5:fl===3?6:7;
    if(fr%_aRate===0){
      const _bpx=20+Math.random()*(W-40),_bpy=30+Math.random()*(H-130);
      const _bc1=fl>=5?'rgba(255,200,60,0.6)':fl===4?'rgba(220,80,20,0.7)':fl===3?'rgba(200,80,200,0.55)':fl===2?'rgba(100,60,180,0.5)':'rgba(110,120,140,0.5)';
      const _bc2=fl>=5?'rgba(80,240,220,0.55)':fl===4?'rgba(255,160,40,0.6)':fl===3?'rgba(140,80,255,0.5)':fl===2?'rgba(150,100,220,0.4)':'rgba(80,90,110,0.45)';
      const _bvy=fl>=4?-0.6-Math.random()*0.8:-0.15-Math.random()*0.3;
      particles.push({x:_bpx,y:_bpy,vx:(Math.random()-.5)*0.4,vy:_bvy,life:fl>=4?12+Math.random()*10:28+Math.random()*20,c:Math.random()>.5?_bc1:_bc2});
    }}
  }
...
// Draws tell accuracy reveal banner; yBase is the top of the 14px banner strip
function _drawAccuracyReveal(px_,yBase,bw_,revT_,acc_,bluffName_,alpha_){
  const revA=revT_<35?Math.min(1,revT_/8):(45-revT_)/10;
  g.globalAlpha=revA*alpha_;
...
function drawOpponentInfoBox(){
  ...
  if(showAccuracyReveal)_drawAccuracyReveal(bx_,by_+34,bw,_revT,bpTellWasAccurate[0],'VEGA',0.92);
  // Separator
  ...
  if(r2alive){
    ...
    if(showAccuracyReveal&&bpRivalTells[1])_drawAccuracyReveal(bx_,sepY+26,bw,_revT,bpTellWasAccurate[1],'MIRA',0.88);
  }
  // Scout intel
  if(hasScout){
    let intelY=by_+bh-4;
...
function drawSelectPhase(){
  ...
  drawParticles(0,0); // v526: battle ambient particles visible during select phase
}
...
function drawConfirmingPhase(){
  ...
  // actionType on-chain: Rust enum is 1-indexed (Draw=1,Steal=2,Barrier=3,Scout=4,UseCrystal=5)
  // bpAction is 0-indexed (DRAW=0,STEAL=1,BARRIER=2,SCOUT=3,USE CARD=4) — add 1 for on-chain
  if(t===1&&walletConnected){
    onchainCommitPhase=true;
    onchainCommit(_sessionGameId,bpAction+1,encounterExclTarget||0).then(result=>{
      if(result){
  ...
  if(t===1&&!walletConnected){
    ...
    computeCommitHash(bpAction+1,new Uint8Array(32),salt).then(hash=>{
  ...
  if(t===70&&walletConnected&&onchainPendingSalt){
    ...
    onchainReveal(_sessionGameId,bpAction+1,encounterExclTarget||0,onchainPendingSalt).then(result=>{
      if(result){
  ...
  drawParticles(0,0); // v526: battle ambient particles visible during confirming phase
}

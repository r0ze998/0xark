...
function drawBattleBG(){
  ...
  if(currentMap===0&&!inDungeon){
    ...
    g.globalAlpha=0.35;
    txShadow(flNames[fl],W/2-30,16,6,flCol,'rgba(0,0,0,.4)');
    g.globalAlpha=1;
  }
    ...
    if(barrierActive){
      ...
      if(recentRounds.length>0){
        ...
        recentRounds.forEach((h,ri)=>{
          ...
          txShadow('V:'+(vStole?'STL':h.r1a===2?'BAR':h.r1a===0?'DRW':'SCT'),ppX+30,hy+10,5,vStole?'#d04040':'#687070','rgba(0,0,0,.3)');
          // M action
          const mStole=h.r2a===1;
          bx(ppX+56,hy,24,14,'rgba(0,0,0,.4)');
          bx(ppX+56,hy,24,1,mStole?'#d04040':'#686060');
          txShadow('M:'+(mStole?'STL':h.r2a===2?'BAR':h.r2a===0?'DRW':'SCT'),ppX+58,hy+10,5,mStole?'#d04040':'#687070','rgba(0,0,0,.3)');
          // Outcome

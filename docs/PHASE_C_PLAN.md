...
| C1-2 | MagicBlock ER: `DELeGGvXpWV2fqJUhqcF5ZSYMS4JTLjteaAMARRSaeSh` へのアカウント委任実装 | **critical** | 8h | — |
...
| C2-5 | `dungeon_position.circom` 設計: `Poseidon(x, y, salt) == posHash` | **important** | 6h | — |
| C2-6 | `dungeon_position` trusted setup + browser proving テスト | **important** | 4h | C2-5 |
| C2-7 | proof 生成をバックグラウンド Worker に移動してメインスレッドをブロックしない | **important** | 3h | C2-3 |
| C2-8 | [r0ze 判断待ち] MagicBlock TEE を位置秘匿の代替として採用するか評価 | **important** | 2h | — |
...
> **[r0ze 判断待ち]**: 位置 ZK サーキットをフルに実装するか、MagicBlock TEE を代替として採用するか？ TEE 採用ならサーキット実装 (C2-5, C2-6) はスキップ可能。
...
- CLAUDE.md: 参加費 0.5 SOL、スターターカード 3 枚
- x402 AI agent ブローカー: USDC マイクロペイメント実装済み

### ギャップ

- シーズン期間未定義
- Prize Pool 清算ロジック（複数コンプ時の処理）未設計
- エントリー費用の通貨（SOL vs USDC）が資料間で不一致

### タスク列

| # | タスク | 優先度 | 見積 | 依存 |
|---|--------|--------|------|------|
| C4-1 | [r0ze 判断待ち] エントリー費用を SOL・USDC どちらで確定するか決定 | **critical** | 1h | — |
| C4-2 | [r0ze 判断待ち] シーズン期間を確定 (推奨: 2–4 週間) | **critical** | 1h | — |
...
| C4-9 | [r0ze 判断待ち] Pyth SOL/USDC 価格フィード採用で Prize Pool を USDC 表示するか | **nice-to-have** | 2h | C4-1 |
...
| Prize Pool | シーズンエントリー手数料累積 | 清算時 100% |
...
| C5-6 | [r0ze 判断待ち] Sonic SVM: Solana mainnet と比較して優位性を評価し採用可否を決定 | **important** | 3h | — |
| C5-7 | [r0ze 判断待ち] Pyth SOL/USDC 価格フィード: Prize Pool の USDC 建て表示に採用するか | **nice-to-have** | 3h | C4-9 |
| C5-8 | スポンサーロゴ + 統合説明をゲーム内 About 画面に追加 | **nice-to-have** | 1h | — |
...
| C6-5 | [r0ze 判断待ち] MIROSS コミュニティとの連携方法を確定 (何コミュニティ？どう巻き込む？) | **important** | 2h | — |
| C6-6 | クローズドベータ: 5/4–5/11 に 10 名テスター招待・フィードバック収集 | **critical** | 3h | — |
| C6-7 | README + GitHub Pages ランディングページ更新 | **important** | 2h | — |
| C6-8 | ハッカソン提出フォーム記入 (説明文・動画リンク・GitHub) | **critical** | 2h | C3-8 |

**合計見積: ~19h**

> **[r0ze 判断待ち]**: MIROSS コミュニティとは何か（Discord コミュニティ？DAOトークン保有者層？）、また 0xARK をどのように紹介・連携するか方針を教えてください。
...
| 4/21 (月) | C1-2 MagicBlock ER アカウント委任実装 | ER 委任コード動作 |
| 4/22 (火) | C1-3 セッションキー UI 統合、C1-4 commit_frequency 確認 | ウォレットポップアップ消滅 |
| 4/23 (水) | C1-1 verify_zk_proof + deposit_stake + claim_prize 分離プログラムデプロイ | devnet に verifier 着地 |
| 4/24 (木) | C2-1 Groth16 verifier devnet デプロイ、C2-2 CPI 連結 | ZK verify フロー動作 |
| 4/25 (金) | C2-3 E2E proof→submit 確認、C5-2 Helius Webhook 設定 | proof submit on-chain 成功 |
...
| 4/29 (火) | C4-1 [r0ze 判断待ち] 費用確定後、C4-5 手数料ロジック実装 | Marketplace 手数料動作 |
...
| 5/3 (土) | C5-3 Helius→WebSocket プッシュ連結、C5-6 [r0ze 判断待ち] Sonic SVM 評価 | リアルタイムイベント表示 |
...
| 5/8 (木) | C6-6 クローズドベータ開始 (10 名招待)、C4-6 Legends システム | ベータフィードバック収集開始 |

### Final Stretch: 提出準備 (5/9–5/11)

| 日付 | 主要タスク | 完了目標 |
|------|-----------|---------|
| 5/9 (金) | 動画ファイル最終化・YouTube/Vimeo アップロード、C6-7 README 更新 | 提出用素材完成 |
| 5/10 (土) | C6-8 ハッカソン提出フォーム記入・プレビュー確認、最終 QA | フォーム記入完了 |
...
**最小 critical path**: C1-1 → C2-1 → C2-2 → C2-3 → C3-6 → C3-8 → C6-8 → 提出
...
| [r0ze 判断待ち] Sonic SVM 評価に時間がかかる | 低 | 5/3 に評価期限を設定。採用しない場合は Solana mainnet で固定 |
| [r0ze 判断待ち] Pyth 統合の工数が想定超え | 低 | Prize Pool の USDC 表示は Pyth なしでも実装可能（静的レート換算で代替） |
| クローズドベータのフィードバックで大きなバグ発見 | 中 | ベータ開始を 5/4 に前倒し、5/8–5/9 を修正バッファとして確保 |
| MIROSS 連携の方向性未確定 | 低 | [r0ze 判断待ち] 確定するまで Farcaster / Twitter 施策を優先して進める |

---

## [r0ze 判断待ち] 一覧

以下は r0ze の判断が確定次第、対応タスクを開始できるアイテムです。

1. **C2-8**: MagicBlock TEE を位置秘匿の代替として採用するか → `dungeon_position.circom` 実装 (C2-5, C2-6) をスキップするか否か
2. **C4-1**: エントリー費用を SOL / USDC どちらで統一するか → Prize Pool 設計・フロントエンド表示に影響
3. **C4-2**: シーズン期間を確定（推奨: 2–4 週間）
4. **C4-9 / C5-7**: Pyth SOL/USDC 価格フィードを Prize Pool USDC 表示に採用するか
5. **C5-6**: Sonic SVM の評価・採用可否 → Solana mainnet で十分か、Sonic SVM に優位性があるか
6. **C6-5**: MIROSS コミュニティとは何か、0xARK との連携方法

---

*このドキュメントは Phase C 開始時点 (2026-04-19) の計画です。状況変化に応じて随時更新してください。*

...
The long-term thesis is not human card gaming. It is AI-to-AI economic behavior at scale. 0xARK ships Phase 1 of that thesis: a Claude Haiku 4.5 agent participates in live duels, making real-time decisions via Anthropic API every WebSocket tick. The agent decides autonomously whether Scout Peek is economically justified given the current board state; the client executes the actual 0.005 SOL x402 transfer. On-chain NFT transfer logic (`record_card_owner_change`, `StealType` enum) is fully implemented. Full agent wallet autonomy — agent holds NFTs, signs its own transactions, pays x402 direct — is scoped for Season 2. 0xARK is the first playable environment where that decision-layer thesis runs on a production blockchain, with the execution layer ready to wire up next.
